...
p <- ggplot(data = mean_replicate) + 
  geom_point(aes(x = beta, y = mean_betahat, color = probe), size = 0.1, alpha = 0.8) +
  theme_bw() +
...
ggsave(p, file = "~/Dropbox/600 Presentations/Yale projects/low_intensity_probe_correction/figs/beta_hat_beta.png", width = 3, height = 3)
